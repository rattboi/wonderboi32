...
#define playbuffer_size 4096
static signed char playbuffer1[playbuffer_size];
static signed char playbuffer2[playbuffer_size];
static signed char *playbuffer_current;
static int playbuffer=2;
...
static
void PcmPlay(unsigned char* src, int size, int rep, int bit) {
   // bit 0 = 8bit
   // bit 1 =16bit
   if (bit==1) size/=2;
...
static void DMA2_Done(void) __attribute__ ((naked));
static void DMA2_Done(void) 
{
	...
	{
	   playbuffer=2;
	   playbuffer_current = playbuffer2;
	   PcmPlay((signed char *)&playbuffer1,playbuffer_size,0, 0);
	}
	else 
	{
	   playbuffer=1;
	   playbuffer_current = playbuffer1;
	   PcmPlay((signed char *)&playbuffer2,playbuffer_size,0, 0);
	}
	
	rSRCPND=BIT_DMA2;   //clear pending bit
	rINTPND=BIT_DMA2;
	
	// fill in other buffer
	
	updatemixing (playbuffer_current, playbuffer_size);
	
	asm volatile ("LDMIA    r13!,{r0-r12,lr}\n"
	             "SUBS PC,lr,#4\n");
}

void gp_initSound( int freq, int bit) {
    int sysfs=0;
	...
	playbuffer_current=playbuffer2;
	PcmPlay(playbuffer_current,4,0,0);
}
