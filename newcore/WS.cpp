...
void __fastcall ErrorMsg(char *Msg)
{
	char str[128];
    int i=nec_get_reg(NEC_CS)<<4;
    int j=nec_get_reg(NEC_IP);

	sprintf(str,"%s %06X", Msg, i+j);
...
const static BYTE2INT ROMBanksTable[] = {
...
int RAMEnable;
const static BYTE2INT RAMBanksTable[] = {
...
const static BYTE2INT RAMSizeTable[] = {
...
const static BYTE2INT RAMKindTable[] = {
	{0x10, CK_EEP},
	{0x20, CK_EEP},
	{NULL, NULL}
};
...
void __fastcall ComEEP(struct EEPROM *Eep, byte *Cmd, byte *Data)
{
	int i, j, cmd, op, addr;
		...
		case 0:
			...
			switch(addr)
			{
				...
				case 2:
					if(Eep->WrEnable)
					{
						memset(Eep->pData,-1, (tblmask[i][4]+1)<<1);
					}
					break;
				case 3:
					Eep->WrEnable=1;
...
byte __fastcall ReadMem(unsigned int A)
{
	return Page[((A)>>16) &0xF][(A) &0xFFFF];
}

void __fastcall WriteMem(unsigned int A, byte V)
{
	(*WriteMemFnTable[(A>>16) &0x0F])(A, (byte) V);
}

typedef void __fastcall (*WriteMemFn) (unsigned int A, byte V);

static void __fastcall WriteRom(unsigned A, byte V)
{
//	if(Verbose)
//	ErrorMsg("ROM닢리어 기입 위반");
}

static void __fastcall WriteIRam(unsigned A, byte V)
{
	IRAM[A&0xFFFF]=V;
	if((A&0xFE00) ==0xFE00)
	{
...
static void __fastcall WriteCRam(unsigned A, byte V)
{
	Page[(A>>16) &0x0F][A&0xFFFF]=V;
}

void __fastcall WriteIO(unsigned A, byte V)
{
	int i, j, k, n;
    byte b;
